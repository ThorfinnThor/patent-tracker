...
export function generateStaticParams() {
  return listCompanyIds("biotech").map(companyId => ({ companyId }));
}
...
export default function BiotechCompanyPage({ params }: { params: { companyId: string } }) {
  ...
  const rows = readCompanyPatents("biotech", params.companyId);

  return (
    <main style={{ display: "grid", gap: 12 }}>
      <a href="/biotech" style={{ textDecoration: "none" }}>← Back</a>
