from __future__ import annotations

import csv
import glob
import json
import os
from dataclasses import dataclass

import pandas as pd


@dataclass(frozen=True)
class BuildConfig:
  sector_id: str
  store_dir: str              # data/store/<sector>/
  out_public_dir: str         # apps/web/public/data/<sector>/
  out_pg_dir: str             # data/state/postgres/


def _safe_int(x) -> int:
  try:
    return int(float(x))
  except Exception:
    return 0


def _clean_text(x) -> str:
  if x is None:
    return ""
  s = str(x)
  # remove embedded newlines/tabs that break CSV COPY semantics
  s = s.replace("\r", " ").replace("\n", " ").replace("\t", " ")
  return s.strip()


def _load_store(store_dir: str, prefix: str) -> pd.DataFrame:
  files = sorted(glob.glob(os.path.join(store_dir, f"{prefix}_*.csv")))
  if not files:
    return pd.DataFrame()
  parts = [pd.read_csv(f, dtype=str) for f in files]
  return pd.concat(parts, ignore_index=True) if parts else pd.DataFrame()


def _write_pg_csv(df: pd.DataFrame, path: str) -> None:
  """
  Write a COPY-safe CSV:
    - all fields quoted
    - consistent newline
    - no index
  """
  os.makedirs(os.path.dirname(path), exist_ok=True)
  df.to_csv(
    path,
    index=False,
    quoting=csv.QUOTE_ALL,
    lineterminator="\n",
    escapechar="\\",
    doublequote=True,
  )


def build_sector_artifacts(cfg: BuildConfig) -> None:
  os.makedirs(cfg.out_public_dir, exist_ok=True)
  os.makedirs(cfg.out_pg_dir, exist_ok=True)

  pairs = _load_store(cfg.store_dir, "pairs")
  if pairs.empty:
    raise RuntimeError(f"No pairs store found under {cfg.store_dir}")

  # Keep only corporations/companies as "tracked companies"
  pairs["assignee_type"] = pairs.get("assignee_type", "").fillna("").astype(str)
  corp = pairs[pairs["assignee_type"] == "2"].copy()

  # Ensure required columns exist
  for col in [
    "patent_id",
    "patent_title",
    "patent_date",
    "patent_num_times_cited_by_us_patents",
    "cpc_subclass_ids",
    "cpc_group_ids",
    "canonical_company_id",
    "display_name",
  ]:
    if col not in corp.columns:
      corp[col] = ""

  # Clean + normalize
  corp["patent_id"] = corp["patent_id"].fillna("").map(_clean_text)
  corp["patent_title"] = corp["patent_title"].fillna("").map(_clean_text)
  corp["patent_date"] = corp["patent_date"].fillna("").map(_clean_text)
  corp["cpc_subclass_ids"] = corp["cpc_subclass_ids"].fillna("").map(_clean_text)
  corp["cpc_group_ids"] = corp["cpc_group_ids"].fillna("").map(_clean_text)
  corp["canonical_company_id"] = corp["canonical_company_id"].fillna("").map(_clean_text)
  corp["display_name"] = corp["display_name"].fillna("").map(_clean_text)

  corp["cited_by"] = corp["patent_num_times_cited_by_us_patents"].fillna("0").map(_safe_int)
  corp["patent_year"] = corp["patent_date"].astype(str).str.slice(0, 4).map(lambda x: _safe_int(x) if x else 0)

  # Company stats
  company_stats = (
    corp.groupby(["canonical_company_id", "display_name"], dropna=False)
      .agg(
        patentCount=("patent_id", "nunique"),
        totalCitations=("cited_by", "sum"),
      )
      .reset_index()
  )
  company_stats["citationsPerPatent"] = company_stats.apply(
    lambda r: (float(r["totalCitations"]) / float(r["patentCount"])) if r["patentCount"] else 0.0,
    axis=1,
  )

  # CPC breadth by subclass
  def _cpc_breadth(series: pd.Series) -> int:
    s: set[str] = set()
    for v in series.fillna("").astype(str):
      for code in v.split("|"):
        code = code.strip()
        if code:
          s.add(code)
    return len(s)

  breadth = (
    corp.groupby(["canonical_company_id"], dropna=False)["cpc_subclass_ids"]
      .apply(_cpc_breadth)
      .reset_index(name="cpcBreadth")
  )
  company_stats = company_stats.merge(breadth, on="canonical_company_id", how="left")
  company_stats["cpcBreadth"] = company_stats["cpcBreadth"].fillna(0).astype(int)

  # Top N companies (default 200, override with TOP_N_COMPANIES)
  top_n = int(os.environ.get("TOP_N_COMPANIES", "200"))
  top = company_stats.sort_values(["patentCount", "totalCitations"], ascending=[False, False]).head(top_n).copy()
  top_ids = set(top["canonical_company_id"].astype(str))

  # Public companies.json
  companies_out = top.rename(columns={"canonical_company_id": "companyId", "display_name": "displayName"})[
    ["companyId", "displayName", "patentCount", "totalCitations", "citationsPerPatent", "cpcBreadth"]
  ].to_dict(orient="records")

  with open(os.path.join(cfg.out_public_dir, "companies.json"), "w", encoding="utf-8") as f:
    json.dump(companies_out, f, indent=2)

  # ---------- Postgres exports ----------
  patents_df = corp[corp["canonical_company_id"].astype(str).isin(top_ids)].copy()
  patents_df["sector"] = cfg.sector_id
  patents_df["company_id"] = patents_df["canonical_company_id"].astype(str)

  # Deduplicate to unique company/patent
  patents_df = patents_df.sort_values(["patent_date", "patent_id"]).drop_duplicates(
    subset=["sector", "company_id", "patent_id"], keep="last"
  )

  # Force exact schema + sanitize text
  pg_patents = patents_df.reindex(
    columns=[
      "sector",
      "company_id",
      "patent_id",
      "patent_date",
      "patent_year",
      "patent_title",
      "cited_by",
      "cpc_subclass_ids",
      "cpc_group_ids",
    ]
  ).copy()

  for c in ["sector", "company_id", "patent_id", "patent_date", "patent_title", "cpc_subclass_ids", "cpc_group_ids"]:
    pg_patents[c] = pg_patents[c].fillna("").map(_clean_text)
  for c in ["patent_year", "cited_by"]:
    pg_patents[c] = pg_patents[c].fillna(0).map(_safe_int)

  _write_pg_csv(pg_patents, os.path.join(cfg.out_pg_dir, f"{cfg.sector_id}_patents.csv"))

  # Companies export for Postgres
  pg_companies = top.rename(columns={"canonical_company_id": "companyId", "display_name": "displayName"})[
    ["companyId", "displayName", "patentCount", "totalCitations", "citationsPerPatent", "cpcBreadth"]
  ].copy()
  pg_companies.insert(0, "sector", cfg.sector_id)

  for c in pg_companies.columns:
    pg_companies[c] = pg_companies[c].fillna("").map(_clean_text)

  _write_pg_csv(pg_companies, os.path.join(cfg.out_pg_dir, f"{cfg.sector_id}_companies.csv"))

  # Inventors export
  inventors = _load_store(cfg.store_dir, "inventors")
  if inventors.empty:
    pd.DataFrame(
      columns=[
        "sector",
        "company_id",
        "patent_id",
        "inventor_id",
        "inventor_name_first",
        "inventor_name_last",
        "inventor_name",
        "patent_date",
      ]
    ).to_csv(os.path.join(cfg.out_pg_dir, f"{cfg.sector_id}_inventors.csv"), index=False, quoting=csv.QUOTE_ALL, lineterminator="\n")
    return

  for col in [
    "canonical_company_id",
    "patent_id",
    "inventor_id",
    "inventor_name_first",
    "inventor_name_last",
    "inventor_name",
    "patent_date",
  ]:
    if col not in inventors.columns:
      inventors[col] = ""

  inventors = inventors[inventors["canonical_company_id"].astype(str).isin(top_ids)].copy()
  inventors["sector"] = cfg.sector_id
  inventors["company_id"] = inventors["canonical_company_id"].astype(str)
  inventors = inventors.drop_duplicates(subset=["sector", "company_id", "patent_id", "inventor_id"])

  pg_inventors = inventors.reindex(
    columns=[
      "sector",
      "company_id",
      "patent_id",
      "inventor_id",
      "inventor_name_first",
      "inventor_name_last",
      "inventor_name",
      "patent_date",
    ]
  ).copy()

  for c in pg_inventors.columns:
    pg_inventors[c] = pg_inventors[c].fillna("").map(_clean_text)

  _write_pg_csv(pg_inventors, os.path.join(cfg.out_pg_dir, f"{cfg.sector_id}_inventors.csv"))
