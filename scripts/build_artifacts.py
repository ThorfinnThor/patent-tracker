...
def _write_pg_csv(df: pd.DataFrame, path: str) -> None:
    """
    Write a Postgres COPY-safe CSV:
      - all fields quoted
      - quotes escaped by doubling (Postgres native for CSV)
      - stable newline
    """
...
def build_sector_artifacts(cfg: BuildConfig) -> None:
    ...
    inv_path = os.path.join(cfg.out_pg_dir, f"{cfg.sector_id}_inventors.csv")
    ...
    _write_pg_csv(pg_inventors, inv_path)
